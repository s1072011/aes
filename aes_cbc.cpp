...
ByteArray AesCBC::Encrypt(ByteArray &&plaintext, ByteArray &&key, AesKeyLengthOptions option,
                          const ByteArray &iv) {
  Init(option);
  ...
  for (size_t i = 0; i < block_count; i++) {
    ...
    auto temp = XorWordArray(priv_block.get(), pt_word_arr.get() + offset);

    Cipher(temp.get());

    std::memcpy(priv_block.get(), temp.get(), BLOCK_SIZE_IN_BYTES);
    std::memcpy(pt_word_arr.get() + offset, temp.get(), BLOCK_SIZE_IN_BYTES);
  }
...
ByteArray AesCBC::Decrypt(ByteArray &&ciphertext, ByteArray &&key, AesKeyLengthOptions option,
                          const ByteArray &iv) {
  Init(option);
  KeyExpansion(std::move(key));
  ...
  ParallelEncryption(&AesCBC::InvCipher, ct_word_arr.get(), block_count);
